...
public class STRTree implements java.io.Serializable{
	...
	public void secureRangeQuery(STRNode n,Rect query,List<Rect> result,List<String> VO)
	{
	...
	public void secureRangeQuery(STRNode n,Circle query,List<Rect> result,List<String> VO)
	{
		if(query.isIntersects(n.MBR)) {
			VO.add("[");
			for(int i =0;i<n.child.size();i++) {
				if(query.isIntersects(n.child.get(i).MBR) && !n.child.get(i).isleaf) {
					...
					if(n.child.get(i).isleaf) {
						VO.add(n.child.get(i).MBR.toString());
						if(query.isIntersects(n.child.get(i).MBR)) {
							//System.out.println("result: "+n.child.get(i).MBR.toString());
							result.add(n.child.get(i).MBR);
						}
					}
	...
	public void nkNN(int k,Point query,LinkedList<Rect> RS,LinkedList<String> VO) {
		STRNode n = this.root;
		NNEntry rootn = new NNEntry(n,getMinimumDist(query,n.MBR));
		Comparator<NNEntry> comp = new Comparator<NNEntry>() {
			@Override
		...
		PriorityQueue<NNEntry> pq = new PriorityQueue<NNEntry>(comp);
		pq.add(rootn);
		int counter = 0;
		while(!pq.isEmpty() && counter < k) {
			...
			if(f.node.isleaf) {
				...
				ArrayList<STRNode> child = f.node.child;
				for(STRNode c:child) {
					pq.add(new NNEntry(c,getMinimumDist(query,c.MBR)));
				}
			}
		}
		Rect kth = RS.get(RS.size()-1);
		LinkedList<Rect> rr = new LinkedList<Rect>();
		double rdist = getMinimumDist(query,kth);
		//System.out.println(rdist);
		secureRangeQuery(this.root,new Circle(query,rdist),rr,VO);
	}
	public void secureKNN(int k,Point query,List<Rect> result,List VO) {
		STRNode n = this.root;
		...
		while(counter<k) {
			...
			if(!t.node.isleaf) {
				...
				VO.remove(minindex);
				VO.add(minindex,t.node.MBR.toString());
				counter++;
				t.dist = 9999999.0;
	...
	public VOreturn RootHash(LinkedList<String> VO) {
		...
		while(!VO.isEmpty()) {
			String f = VO.poll();
			if((f.charAt(0)>='0' && f.charAt(0)<='9')||f.charAt(0)=='-') {
				str = str + f;
